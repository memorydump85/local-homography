...
from raymap.graph import IntrinsicsNode, ExtrinsicsNode, PureProjectionEdge, PureProjectionTestEdge
...
#--------------------------------------    
    sse, n = 0, 0
    for e in filter(lambda x: isinstance(x, PureProjectionEdge), g.edges):
        sse += e.getChi2(g)
        n += e.getDOF();
    return sse/n        
...
#   All parameters are JSON dictionary objects
    ...
    g = Graph()
    g.nodes.add(inode)
    complete_graph(g, tag_correspondences, PureProjectionEdge, lambda x : x)
    ...
    gtest = Graph()
    gtest.nodes.add(inode)
    complete_graph(gtest, test_correspondences, PureProjectionTestEdge, lambda x : [x[1]])
